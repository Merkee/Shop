...
const ProductsLoader = (props) => {
    ...
    useEffect(() => setProducts(props.productsData.products), [props.productsData.products]);
    if(!products.length){
        return <p onClick={() => console.log(props.searchParams)}>Продукция не найдена.</p>
    }
    return products.map( p => <ProductFrame id={p.id} image={p.image} name={p.name} oldcost={p.cost} cost={(p.cost-p.cost*(p.discount/100)).toFixed(2)}/>);
}
